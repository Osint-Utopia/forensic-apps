...
def export_complete_package(original_pil, analysis_results, base_filename="forensic_analysis"):
    """Export complete analysis package (PNG visualization, DOCX report)"""
    print(f"\n{'='*80}")
    ...
    try:
        ...
        # 2. Export DOCX report (jika python-docx tersedia)
        if DOCX_AVAILABLE:
            ...
            print("  Skipping DOCX report generation as python-docx is not installed.")
...
def export_comprehensive_package(original_pil, analysis_results, base_filename="forensic_analysis"):
    ...
    try:
        ...
        if DOCX_AVAILABLE:
            ...
            export_files['docx_report'] = export_to_advanced_docx(original_pil, analysis_results, docx_file)
            # PDF generation removed - not supported in Hugging Face
        else:
            print("  Skipping DOCX report generation as python-docx is not installed.")

        # 5. Create index HTML file (inside the specific package dir)
        ...
        # 6. Create ZIP archive of everything
...
def add_dfrws_examination_section(doc, analysis_results, original_pil):
    ...
    doc.add_paragraph(
        ...
        "17 visualisasi utama yang terbagi dalam tiga kategori: Analisis Inti, Analisis Lanjut, "
    ...
    # 0. Metadata Analysis Table (NEW)
    doc.add_heading('4.1.0 Analisis Metadata Forensik', level=3)
    doc.add_paragraph(
        "Metadata adalah informasi tersembunyi dalam file gambar yang berisi detail teknis tentang "
        "bagaimana gambar dibuat, dimodifikasi, dan disimpan. Analisis metadata forensik sangat penting "
        "karena dapat mengungkap inkonsistensi yang mengindikasikan manipulasi, seperti perbedaan "
        "timestamp, software yang digunakan, atau informasi kamera yang tidak sesuai."
    )
    ...
    if MATPLOTLIB_AVAILABLE:
        try:
            ...
            from visualization import create_metadata_table
            create_metadata_table(ax, analysis_results.get('metadata', {}))
            buf = io.BytesIO()
            ...
            metadata_score = analysis_results.get('metadata', {}).get('Metadata_Authenticity_Score', 'N/A')
            doc.add_paragraph(
                f"Gambar 0: Tabel analisis metadata forensik. Skor autentisitas metadata: {metadata_score}",
                style='Caption'
            )
            
            # Add metadata interpretation
            metadata_inconsistencies = analysis_results.get('metadata', {}).get('Metadata_Inconsistency', [])
            if metadata_inconsistencies and metadata_inconsistencies != ['Tidak ada']:
                doc.add_paragraph(
                    f"⚠️ Terdeteksi inkonsistensi metadata: {', '.join(metadata_inconsistencies)}. "
                    "Ini dapat mengindikasikan bahwa gambar telah dimodifikasi atau diproses ulang."
                )
            else:
                doc.add_paragraph(
                    "✅ Tidak terdeteksi inkonsistensi signifikan dalam metadata. "
                    "Namun, metadata dapat dengan mudah dimanipulasi, sehingga analisis teknis lebih lanjut tetap diperlukan."
                )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Metadata: {e}", style='Warning')
            print(f"Error creating Metadata visualization for DOCX: {e}")
    else:
        doc.add_paragraph("Matplotlib tidak tersedia untuk visualisasi metadata.")
    ...
    doc.add_paragraph(
        ...
        "manipulasi gambar digital. Metode ini bekerja dengan prinsip bahwa setiap kali gambar JPEG "
        "disimpan, ia mengalami kompresi lossy yang menghasilkan artefak tertentu. Area yang telah "
        "dimanipulasi (seperti objek yang ditambahkan dari gambar lain) akan memiliki riwayat kompresi "
        "yang berbeda dari area asli, sehingga menghasilkan pola error yang dapat dideteksi."
    )
    ...
    doc.add_paragraph(
        "1. Gambar asli dikompres ulang pada kualitas JPEG standar (biasanya 90-95%)",
        style='List Number'
    )
    doc.add_paragraph(
        "2. Perbedaan pixel antara gambar asli dan hasil kompresi dihitung",
        style='List Number'
    )
    doc.add_paragraph(
        "3. Perbedaan ini divisualisasikan sebagai heatmap dengan skala warna",
        style='List Number'
    )
    ...
    doc.add_paragraph(
        "• **Area terang (putih/merah):** Tingkat error tinggi - kemungkinan area yang baru ditambahkan, "
        "dimodifikasi, atau berasal dari sumber dengan kualitas kompresi berbeda",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Area gelap (hitam/biru):** Tingkat error rendah - kemungkinan area asli yang tidak mengalami "
        "manipulasi atau sudah mencapai keseimbangan kompresi",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Pola tidak seragam:** Variasi signifikan dalam intensitas ELA di berbagai area gambar "
        "dapat mengindikasikan manipulasi, terutama jika membentuk pola yang tidak natural",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Keterbatasan:** ELA dapat menghasilkan false positive pada area dengan detail tinggi, "
        "tepi tajam, atau gradien warna yang kompleks",
        style='List Bullet'
    )
    ...
    doc.add_heading('4.1.5 Lokalisasi Area Mencurigakan', level=3)
    doc.add_paragraph(
        "Analisis lokalisasi menggunakan algoritma K-Means clustering untuk mengidentifikasi dan "
        "memvisualisasikan area yang kemungkinan telah dimanipulasi. Area yang ditandai dengan warna "
        "merah menunjukkan region dengan karakteristik forensik yang mencurigakan berdasarkan kombinasi "
        "dari berbagai indikator seperti ELA, noise inconsistency, dan kecocokan fitur."
    )
    
    # Create localization visualization
    if MATPLOTLIB_AVAILABLE and 'localization_analysis' in analysis_results:
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            ...
            doc.add_paragraph(
                f"Gambar 5: Visualisasi lokalisasi area mencurigakan. "
                f"Persentase area terdeteksi manipulasi: {tampering_pct:.2f}%",
                style='Caption'
            )
            
            if tampering_pct > 0:
                doc.add_paragraph(
                    f"Analisis K-Means mendeteksi sekitar {tampering_pct:.2f}% dari total area gambar "
                    f"memiliki karakteristik yang tidak konsisten dengan area lainnya. Ini bisa mengindikasikan "
                    f"adanya manipulasi, meskipun perlu diverifikasi dengan metode lain."
                )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Lokalisasi: {e}", style='Warning')
            print(f"Error creating Localization visualization for DOCX: {e}")
    else:
        doc.add_paragraph("Data lokalisasi area mencurigakan tidak tersedia.")
    
    # ======================= TAHAP 2: ANALISIS LANJUT =======================
    doc.add_page_break()
    doc.add_heading('4.2 Tahap 2: Analisis Lanjut (Advanced Analysis)', level=2)
    doc.add_paragraph(
        "Analisis lanjut menyelidiki properti intrinsik gambar yang lebih dalam seperti domain frekuensi, "
        "konsistensi tekstur, analisis tepi, artefak kompresi, dan deteksi pemalsuan menggunakan metode "
        "pembelajaran mesin. Berikut adalah 7 visualisasi dari analisis lanjut:"
    )
    
    # 6. Frequency Domain Analysis
    doc.add_heading('4.2.1 Analisis Domain Frekuensi', level=3)
    doc.add_paragraph(
        "Analisis domain frekuensi menggunakan Discrete Cosine Transform (DCT) untuk memeriksa "
        "distribusi energi pada berbagai frekuensi. Manipulasi gambar sering mengubah pola frekuensi "
        "normal, terutama pada frekuensi tinggi yang berkaitan dengan detail dan tepi gambar."
    )
    
    if MATPLOTLIB_AVAILABLE and 'frequency_analysis' in analysis_results:
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            ...
            doc.add_paragraph(
                f"Gambar 6: Distribusi energi frekuensi (rendah, sedang, tinggi). "
                f"Inkonsistensi frekuensi: {freq_inconsistency:.3f}",
                style='Caption'
            )
            doc.add_paragraph(
                "Grafik menunjukkan distribusi energi pada tiga band frekuensi. Anomali dalam distribusi "
                "ini dapat mengindikasikan manipulasi, terutama jika ada perbedaan signifikan dari pola normal."
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Domain Frekuensi: {e}", style='Warning')
    
    # 7. Texture Consistency Analysis
    doc.add_heading('4.2.2 Analisis Konsistensi Tekstur', level=3)
    doc.add_paragraph(
        "Analisis tekstur memeriksa konsistensi pola tekstur di seluruh gambar menggunakan metrik "
        "seperti kontras, korelasi, energi, dan homogenitas. Area yang dimanipulasi sering memiliki "
        "karakteristik tekstur yang berbeda dari area asli."
    )
    
    if MATPLOTLIB_AVAILABLE and 'texture_analysis' in analysis_results:
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            from visualization import create_texture_visualization
            ...
            texture_inconsistency = texture_analysis.get('overall_inconsistency', 0)
            doc.add_paragraph(
                f"Gambar 7: Analisis konsistensi tekstur. "
                f"Inkonsistensi tekstur global: {texture_inconsistency:.3f}",
                style='Caption'
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Tekstur: {e}", style='Warning')
    
    # 8. Edge Consistency Analysis
    doc.add_heading('4.2.3 Analisis Konsistensi Tepi', level=3)
    doc.add_paragraph(
        "Analisis tepi menggunakan filter Canny untuk mendeteksi tepi dalam gambar. Manipulasi "
        "sering menyebabkan diskontinuitas atau inkonsistensi pada tepi objek, terutama di area "
        "yang telah di-splice atau di-copy-move."
    )
    
    if MATPLOTLIB_AVAILABLE:
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            from visualization import create_edge_visualization
            ...
            doc.add_paragraph(
                f"Gambar 8: Visualisasi deteksi tepi. "
                f"Inkonsistensi tepi: {edge_inconsistency:.3f}",
                style='Caption'
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Tepi: {e}", style='Warning')
    
    # 9. Illumination Consistency Analysis
    doc.add_heading('4.2.4 Analisis Konsistensi Iluminasi', level=3)
    doc.add_paragraph(
        "Analisis iluminasi memeriksa konsistensi pencahayaan di seluruh gambar. Objek yang "
        "ditambahkan dari sumber berbeda sering memiliki karakteristik pencahayaan yang tidak "
        "konsisten dengan scene asli."
    )
    
    if MATPLOTLIB_AVAILABLE:
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            from visualization import create_illumination_visualization
            ...
            doc.add_picture(buf, width=Inches(5.0))
            illum_analysis = analysis_results.get('illumination_analysis', {})
            illum_inconsistency = illum_analysis.get('overall_illumination_inconsistency', 0)
            doc.add_paragraph(
                f"Gambar 9: Peta konsistensi iluminasi. "
                f"Inkonsistensi iluminasi: {illum_inconsistency:.3f}",
                style='Caption'
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Iluminasi: {e}", style='Warning')
    
    # 10. MM Fusion Analysis
    doc.add_heading('4.2.5 MM Fusion: Deteksi Pemalsuan Multi-Modal', level=3)
    doc.add_paragraph(
        "MM Fusion (Multi-Modal Fusion) adalah metode deteksi pemalsuan canggih yang menggunakan "
        "pendekatan pembelajaran mesin untuk menggabungkan berbagai modalitas analisis forensik. "
        "Sistem ini mengintegrasikan informasi dari multiple domain analysis termasuk pola noise, "
        "karakteristik domain frekuensi, diskontinuitas tepi, anomali ruang warna, dan analisis "
        "pola statistik untuk memberikan penilaian komprehensif tentang keaslian gambar."
    )
    
    doc.add_paragraph(
        "**Keunggulan MM Fusion:**"
    )
    doc.add_paragraph(
        "• **Multi-domain Analysis:** Menganalisis gambar dari berbagai perspektif secara simultan",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Machine Learning Integration:** Menggunakan algoritma AI untuk pattern recognition yang lebih akurat",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Robust Detection:** Mengurangi false positive dengan validasi silang antar modalitas",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Confidence Scoring:** Memberikan skor kepercayaan yang dapat dikuantifikasi",
        style='List Bullet'
    )
    
    mm_fusion_results = analysis_results.get('mm_fusion_analysis', {})
    if MATPLOTLIB_AVAILABLE and mm_fusion_results and 'error' not in mm_fusion_results:
        try:
            ...
            metrics_text = f"""METRIK MM FUSION:
            
Keandalan Deteksi: {confidence_level}
Pemalsuan Terdeteksi: {'Ya' if forgery_detected else 'Tidak'}
Confidence Score: {forgery_confidence:.1f}%
            
Metodologi:
Analisis fusi multi-modal yang
menggabungkan pola noise,
domain frekuensi, konsistensi
tepi, korelasi warna, dan
analisis iluminasi."""
            ...
            doc.add_picture(buf, width=Inches(6.0))
            doc.add_paragraph(
                f"Gambar 10: Analisis MM Fusion. Confidence Score: {forgery_confidence:.1f}%, "
                f"Pemalsuan {'terdeteksi' if forgery_detected else 'tidak terdeteksi'}",
                style='Caption'
            )
            
            # Add detailed explanation
            doc.add_paragraph(
                f"MM Fusion menghasilkan confidence score {forgery_confidence:.1f}% dengan tingkat "
                f"keandalan '{confidence_level}'. {'Sistem mendeteksi indikasi pemalsuan pada gambar ini.' if forgery_detected else 'Sistem tidak mendeteksi indikasi pemalsuan yang signifikan.'}"
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi MM Fusion: {e}", style='Warning')
    else:
        doc.add_paragraph("Data MM Fusion tidak tersedia.")
    
    # 11. TruFor Analysis
    doc.add_heading('4.2.6 TruFor: Analisis Forensik Terpercaya', level=3)
    doc.add_paragraph(
        "TruFor (Trusted Forensics) adalah sistem analisis forensik state-of-the-art yang "
        "menggunakan pendekatan deep learning dan analisis multi-skala untuk deteksi manipulasi "
        "gambar. Sistem ini dikembangkan khusus untuk memberikan penilaian keaslian gambar "
        "dengan tingkat akurasi dan kepercayaan yang sangat tinggi melalui analisis komprehensif "
        "terhadap artefak kompresi, konsistensi pola, dan karakteristik forensik lainnya."
    )
    ...
    doc.add_paragraph(
        "• **Multi-Scale Analysis:** Analisis pada berbagai resolusi untuk deteksi manipulasi halus",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Deep Learning Architecture:** Menggunakan neural network yang dilatih khusus untuk forensik digital",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Compression Artifact Detection:** Deteksi sophisticated terhadap jejak kompresi yang tidak konsisten",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Localization Mapping:** Kemampuan untuk melokalisasi area spesifik yang dimanipulasi",
        style='List Bullet'
    )
    doc.add_paragraph(
        "• **Risk Assessment:** Penilaian tingkat risiko berdasarkan confidence level yang terukur",
        style='List Bullet'
    )
    
    trufor_results = analysis_results.get('trufor_analysis', {})
    if MATPLOTLIB_AVAILABLE and trufor_results and 'error' not in trufor_results:
        try:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
            
            # Left: Forgery localization map
            original_array = np.array(original_pil)
            ax1.imshow(original_array)
            ...
            metrics_text = f"""METRIK TRUFOR:
            
Status: {'Autentik' if is_authentic else 'Mencurigakan'}
Forensic Confidence: {forensic_confidence:.1f}%
Risk Level: {risk_level}
            
Metodologi:
Analisis forensik multi-skala
dengan deteksi artefak
kompresi dan verifikasi
konsistensi pola."""
            ...
            doc.add_picture(buf, width=Inches(6.0))
            doc.add_paragraph(
                f"Gambar 11: Analisis TruFor. Forensic Confidence: {forensic_confidence:.1f}%, "
                f"Status: {'Autentik' if is_authentic else 'Mencurigakan'}",
                style='Caption'
            )
            
            doc.add_paragraph(
                f"TruFor menghasilkan forensic confidence {forensic_confidence:.1f}% dengan risk level "
                f"'{risk_level}'. {'Gambar dinilai autentik dengan tingkat kepercayaan tinggi.' if is_authentic else 'Gambar menunjukkan tanda-tanda manipulasi yang perlu investigasi lebih lanjut.'}"
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi TruFor: {e}", style='Warning')
    else:
        doc.add_paragraph("Data TruFor tidak tersedia.")
    
    # 12. JPEG Ghost Analysis
    doc.add_heading('4.2.7 Analisis JPEG Ghost', level=3)
    doc.add_paragraph(
        "JPEG Ghost analysis mendeteksi area yang telah mengalami tingkat kompresi berbeda. "
        "Ketika bagian gambar ditambahkan dari sumber lain, area tersebut sering memiliki "
        "'ghost' atau jejak kompresi yang berbeda dari gambar asli."
    )
    
    jpeg_ghost_display = analysis_results.get('jpeg_ghost')
    jpeg_analysis = analysis_results.get('jpeg_analysis', {})
    ghost_analysis = jpeg_analysis.get('ghost_analysis', {})
    
    if MATPLOTLIB_AVAILABLE and isinstance(jpeg_ghost_display, np.ndarray):
        try:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
            ...
            ax1.set_title('Peta Deteksi JPEG Ghost')
            ax1.axis('off')
            cbar1 = fig.colorbar(im1, ax=ax1, fraction=0.046, pad=0.04)
            cbar1.set_label('Intensitas Ghost', rotation=270, labelpad=15)
            ...
            if quality_analysis:
                ...
                if qualities:
                    ...
                    ax2.plot(qualities, mean_responses, 'b-o', linewidth=2, markersize=8)
                    ax2.set_xlabel('JPEG Quality')
                    ax2.set_ylabel('Mean Response')
                    ax2.set_title('Respons terhadap Kualitas JPEG')
                    ax2.grid(True, alpha=0.3)
                else:
                    ax2.text(0.5, 0.5, 'Data tidak tersedia', ha='center', va='center')
                    ax2.axis('off')
            else:
                ax2.text(0.5, 0.5, 'Analisis kualitas tidak tersedia', ha='center', va='center')
                ax2.axis('off')
            ...
            doc.add_paragraph(
                f"Gambar 12: Analisis JPEG Ghost. Ghost Score: {ghost_score:.3f}",
                style='Caption'
            )
            
            doc.add_paragraph(
                f"Analisis JPEG Ghost menghasilkan score {ghost_score:.3f}. "
                f"{'Score tinggi (>0.5) mengindikasikan kemungkinan area dengan kompresi berbeda.' if ghost_score > 0.5 else 'Score rendah mengindikasikan konsistensi kompresi di seluruh gambar.'}"
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi JPEG Ghost: {e}", style='Warning')
    else:
        doc.add_paragraph("Data JPEG Ghost tidak tersedia.")
    
    # 13. JPEG Quality Response Analysis (NEW)
    doc.add_heading('4.2.8 Analisis Respons Kualitas JPEG', level=3)
    doc.add_paragraph(
        "Analisis respons kualitas JPEG mengevaluasi bagaimana gambar merespons kompresi ulang "
        "pada berbagai tingkat kualitas. Gambar yang telah dimanipulasi sering menunjukkan pola "
        "respons yang tidak normal, terutama pada kualitas yang mendekati kualitas asli gambar. "
        "Grafik ini membantu mengidentifikasi kualitas JPEG asli dan mendeteksi kompresi ganda."
    )
    
    if MATPLOTLIB_AVAILABLE and 'jpeg_analysis' in analysis_results:
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            from visualization import create_quality_response_plot
            ...
            doc.add_picture(buf, width=Inches(5.0))
            jpeg_basic = analysis_results.get('jpeg_analysis', {}).get('basic_analysis', {})
            estimated_quality = jpeg_basic.get('estimated_original_quality', 'N/A')
            double_compression = jpeg_basic.get('double_compression_indicator', 0)
            doc.add_paragraph(
                f"Gambar 13: Grafik respons kualitas JPEG. Kualitas estimasi: {estimated_quality}, "
                f"Indikator kompresi ganda: {double_compression:.3f}",
                style='Caption'
            )
            
            doc.add_paragraph(
                f"Analisis menunjukkan kualitas JPEG estimasi sebesar {estimated_quality} dengan "
                f"indikator kompresi ganda {double_compression:.3f}. "
                f"{'Nilai tinggi (>0.5) mengindikasikan kemungkinan kompresi ganda yang dapat terjadi saat manipulasi.' if double_compression > 0.5 else 'Nilai rendah mengindikasikan kompresi tunggal yang normal.'}"
            )
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Quality Response: {e}", style='Warning')
            print(f"Error creating Quality Response visualization for DOCX: {e}")
    else:
        doc.add_paragraph("Data analisis kualitas JPEG tidak tersedia atau Matplotlib tidak terinstal.")
    
    # Additional examinations dari analisis lanjut
    doc.add_heading('4.3 Ringkasan Metrik Analisis Lanjut', level=2)
    ...
    # Add comprehensive visualization summary (NEW)
    doc.add_heading('4.4 Ringkasan Komprehensif Visualisasi Forensik', level=2)
    doc.add_paragraph(
        "Laporan ini telah menyajikan analisis forensik komprehensif melalui 17+ visualisasi utama "
        "yang mencakup berbagai aspek deteksi manipulasi gambar. Berikut adalah ringkasan lengkap "
        "dari semua visualisasi yang telah disertakan dalam laporan ini:"
    )
    
    # Create summary table of all visualizations
    viz_table = doc.add_table(rows=1, cols=3)
    viz_table.style = 'Table Grid'
    hdr_cells = viz_table.rows[0].cells
    hdr_cells[0].text = 'No.'
    hdr_cells[1].text = 'Visualisasi'
    hdr_cells[2].text = 'Tujuan Forensik'
    
    visualizations = [
        ('0', 'Analisis Metadata Forensik', 'Deteksi inkonsistensi informasi file dan timestamp'),
        ('1', 'Gambar Asli', 'Referensi baseline untuk semua analisis'),
        ('2', 'Error Level Analysis (ELA)', 'Deteksi area dengan tingkat kompresi berbeda'),
        ('3', 'Feature Matching', 'Identifikasi duplikasi fitur menggunakan SIFT'),
        ('4', 'Block Matching', 'Deteksi copy-move pada level blok piksel'),
        ('5', 'Lokalisasi Area Mencurigakan', 'Pemetaan region yang kemungkinan dimanipulasi'),
        ('6', 'Analisis Domain Frekuensi', 'Deteksi anomali dalam distribusi frekuensi'),
        ('7', 'Konsistensi Tekstur', 'Evaluasi keseragaman pola tekstur'),
        ('8', 'Konsistensi Tepi', 'Deteksi diskontinuitas pada tepi objek'),
        ('9', 'Konsistensi Iluminasi', 'Analisis keseragaman pencahayaan'),
        ('10', 'MM Fusion Analysis', 'Deteksi multi-modal menggunakan AI'),
        ('11', 'TruFor Analysis', 'Analisis forensik terpercaya multi-skala'),
        ('12', 'JPEG Ghost Analysis', 'Deteksi jejak kompresi berganda'),
        ('13', 'Quality Response Plot', 'Evaluasi respons terhadap rekompresi'),
        ('14', 'Probabilitas & Ketidakpastian', 'Analisis kepercayaan dan konsistensi hasil'),
        ('15', 'Analisis Statistik', 'Metrik kuantitatif karakteristik gambar'),
        ('16', 'Lokalisasi K-Means', 'Clustering area berdasarkan karakteristik forensik'),
        ('17', 'Peta Kecurigaan Gabungan', 'Integrasi semua metode deteksi')
    ]
    
    for viz_num, viz_name, viz_purpose in visualizations:
        row_cells = viz_table.add_row().cells
        row_cells[0].text = viz_num
        row_cells[1].text = viz_name
        row_cells[2].text = viz_purpose
    
    doc.add_paragraph(
        "Setiap visualisasi di atas memberikan perspektif unik dalam analisis forensik dan "
        "berkontribusi pada kesimpulan akhir. Kombinasi dari semua metode ini memberikan "
        "analisis yang komprehensif dan dapat diandalkan untuk deteksi manipulasi gambar."
    )
...
def add_dfrws_analysis_section(doc, analysis_results, original_pil):
    ...
    # Probability and Uncertainty Analysis (NEW)
    doc.add_heading('5.4 Analisis Probabilitas dan Ketidakpastian', level=2)
    doc.add_paragraph(
        "Analisis probabilitas menunjukkan tingkat kepercayaan sistem terhadap berbagai jenis manipulasi, "
        "sementara analisis ketidakpastian mengukur seberapa konsisten bukti-bukti forensik yang ditemukan. "
        "Kedua analisis ini sangat penting untuk memberikan konteks yang tepat terhadap hasil deteksi."
    )
    
    # Add probability bars visualization
    classification = analysis_results.get('classification', {})
    if MATPLOTLIB_AVAILABLE and 'uncertainty_analysis' in classification:
        try:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
            
            # Left: Probability bars
            from visualization import create_probability_bars
            create_probability_bars(ax1, analysis_results)
            
            # Right: Uncertainty visualization
            ...
            uncertainty_data = classification.get('uncertainty_analysis', {})
            uncertainty_level = uncertainty_data.get('uncertainty_level', 0)
            indicator_coherence = uncertainty_data.get('indicator_coherence', 'N/A')
            
            doc.add_paragraph(
                f"Gambar 14: Analisis probabilitas dan ketidakpastian. Tingkat ketidakpastian: {uncertainty_level:.1%}, "
                f"Koherensi indikator: {indicator_coherence}",
                style='Caption'
            )
            
            # Add interpretation
            if uncertainty_level < 0.35:
                uncertainty_interpretation = "rendah, menunjukkan konsistensi tinggi antar metode deteksi"
                uncertainty_color = "hijau"
            elif uncertainty_level < 0.65:
                uncertainty_interpretation = "sedang, menunjukkan beberapa inkonsistensi antar metode"
                uncertainty_color = "kuning"
            else:
                uncertainty_interpretation = "tinggi, menunjukkan inkonsistensi signifikan antar metode"
                uncertainty_color = "merah"
            
            doc.add_paragraph(
                f"Tingkat ketidakpastian {uncertainty_level:.1%} dikategorikan sebagai {uncertainty_interpretation}. "
                f"Koherensi indikator '{indicator_coherence}' menunjukkan seberapa konsisten berbagai metode deteksi "
                f"dalam memberikan bukti. Semakin tinggi koherensi, semakin dapat diandalkan hasil analisis."
            )
            
        except Exception as e:
            doc.add_paragraph(f"Tidak dapat memuat visualisasi Probabilitas dan Ketidakpastian: {e}", style='Warning')
            print(f"Error creating Probability/Uncertainty visualization for DOCX: {e}")
    else:
        doc.add_paragraph("Data analisis probabilitas dan ketidakpastian tidak tersedia atau Matplotlib tidak terinstal.")
...
def add_conclusion_advanced(doc, analysis_results):
    """Add comprehensive conclusion with visualization summary"""
    doc.add_heading('6. Kesimpulan', level=1)
    
    # Add note about comprehensive visualizations
    doc.add_paragraph(
        "**Catatan Penting tentang Visualisasi:** Laporan ini telah dilengkapi dengan 17+ visualisasi "
        "forensik komprehensif yang mencakup semua aspek analisis, mulai dari metadata forensik, "
        "Error Level Analysis, deteksi copy-move, analisis domain frekuensi, hingga analisis "
        "probabilitas dan ketidakpastian. Setiap visualisasi disertai dengan penjelasan teknis "
        "yang mendalam untuk membantu interpretasi hasil analisis forensik."
    )
    doc.add_paragraph("")
    classification = analysis_results.get('classification', {})
...
<head>
    ...
    <title>Laporan Forensik Digital - {os.path.basename(getattr(original_pil, 'filename', 'Hasil_Analisis') or 'Hasil_Analisis')}</title>
    <style>
        body {{
...
<body>
    <header>
        ...
        <p>Nama File: {os.path.basename(getattr(original_pil, 'filename', 'Tidak diketahui') or 'Tidak diketahui')}</p>
