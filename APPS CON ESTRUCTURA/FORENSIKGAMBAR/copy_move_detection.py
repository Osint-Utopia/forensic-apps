...
def advanced_tampering_localization(image_pil, ela_array, matches=None, keypoints=None, n_clusters=8):
    ...
    try:
        ...
        # 4. Feature-based validation (if matches are provided)
        if matches and keypoints:
            ...
            for match in matches:
                ...
                # Add confidence around matched keypoints
                for kp in [kp1, kp2]:
                    x, y = int(kp.pt[0]), int(kp.pt[1])
                    if 0 <= x < width and 0 <= y < height:
                        radius = max(5, int(kp.size / 2))
                        y_start = max(0, y - radius)
                        y_end = min(height, y + radius + 1)
                        x_start = max(0, x - radius)
                        x_end = min(width, x + radius + 1)
                        
                        match_confidence = 1.0 - (match.distance / 200.0)  # Normalize distance
                        match_confidence_map[y_start:y_end, x_start:x_end] += match_confidence * 0.4
            
            # Combine with localization map
            localization_map += match_confidence_map
            tampering_confidence += match_confidence_map
...
def kmeans_tampering_localization(image_pil, ela_array, n_clusters=8, matches=None, keypoints=None):
    ...
    return {
        ...
        'tampering_mask': result['tampering_mask'],
        'suspicious_clusters': [],  # Not directly applicable in new approach
        'cluster_means': []  # Not directly applicable in new approach
     }
