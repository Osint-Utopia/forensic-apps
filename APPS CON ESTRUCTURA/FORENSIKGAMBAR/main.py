...
def advanced_tampering_localization(image_pil, analysis_results, n_clusters=8):
    ...
    # Pass matches and keypoints if available from previous feature detection
    matches = analysis_results.get('ransac_matches', [])
    keypoints = analysis_results.get('sift_keypoints', [])
    kmeans_result = kmeans_tampering_localization(image_pil, ela_array, n_clusters=n_clusters, 
    ...
    # 2. Threshold-based localization from ELA dengan selektivitas lebih baik
    ...
    # Dynamic threshold yang lebih selektif untuk menghindari over-detection
    # Gunakan 2.5 standard deviations untuk hanya menangkap anomali signifikan
    ela_threshold = np.clip(ela_mean + 2.5 * ela_std, 10, 200)
    
    # Alternative: gunakan percentile-based threshold yang lebih tinggi
    percentile_threshold = np.percentile(ela_array, 90)  # Hanya top 10% yang suspicious
    
    # Ambil threshold yang lebih tinggi untuk lebih selektif
    final_threshold = max(ela_threshold, percentile_threshold)
    threshold_mask = ela_array > final_threshold
    ...
    tampering_pixels = np.sum(cleaned_mask)
    total_pixels = cleaned_mask.size
    tampering_percentage = (tampering_pixels / total_pixels) * 100 if total_pixels > 0 else 0
    
    # Debug logging
    print(f"  -> K-Means clusters: {n_clusters}, Suspicious clusters: {len(kmeans_result.get('suspicious_clusters', []))}")
    print(f"  -> Tampering pixels: {tampering_pixels}/{total_pixels} = {tampering_percentage:.2f}%")
    print(f"  -> K-means mask coverage: {np.sum(kmeans_mask)}/{kmeans_mask.size} = {(np.sum(kmeans_mask)/kmeans_mask.size)*100:.2f}%")
    print(f"  -> Threshold mask coverage: {np.sum(threshold_mask)}/{threshold_mask.size} = {(np.sum(threshold_mask)/threshold_mask.size)*100:.2f}%")
    print(f"  -> Combined mask coverage: {np.sum(combined_mask)}/{combined_mask.size} = {(np.sum(combined_mask)/combined_mask.size)*100:.2f}%")
