...
def visualize_results_advanced(original_pil, analysis_results, output_filename="advanced_forensic_analysis.png"):
    """Advanced visualization with ALL forensic analysis results including MM Fusion and TruFor"""
    if not MATPLOTLIB_AVAILABLE:
        print("❌ Matplotlib not available. Cannot generate visualization.")
        return None
    print("📊 Creating comprehensive forensic visualization with ALL 22 analysis results...")
    
    # Create figure with optimized size for 22 visualizations (6 rows x 4 columns)
    fig = plt.figure(figsize=(16, 20))  # Compact size to avoid memory issues
    gs = fig.add_gridspec(6, 4, hspace=0.5, wspace=0.3)  # 6x4 grid with tight spacing
    ...
    ax5.set_title("5. K-Means Tampering Localization", fontsize=10, fontweight='bold')
    
    # 6. Edge Consistency Analysis
    ax6 = fig.add_subplot(gs[1, 1])
    create_edge_visualization(ax6, original_pil, analysis_results)
    ax6.set_title("6. Edge Consistency Analysis", fontsize=10, fontweight='bold')
    
    # 7. Illumination Consistency
    ax7 = fig.add_subplot(gs[1, 2])
    create_illumination_visualization(ax7, original_pil, analysis_results)
    ax7.set_title("7. Illumination Map", fontsize=10, fontweight='bold')
    
    # 8. JPEG Ghost Analysis (Enhanced)
    ax8 = fig.add_subplot(gs[1, 3])
    jpeg_ghost = analysis_results.get('jpeg_ghost')
    if jpeg_ghost is not None and isinstance(jpeg_ghost, np.ndarray):
        # Ensure it's not all zeros
        if np.any(jpeg_ghost > 0):
            # Normalize for better visualization
            ghost_normalized = (jpeg_ghost - np.min(jpeg_ghost)) / (np.max(jpeg_ghost) - np.min(jpeg_ghost) + 1e-9)
            ax8.imshow(ghost_normalized, cmap='hot', vmin=0, vmax=1)
            ax8.set_title(f"8. JPEG Ghost (Max: {np.max(jpeg_ghost):.2f})", fontsize=10, fontweight='bold')
        else:
            # If all zeros, try to generate a simple ghost map
            try:
                gray_img = np.array(original_pil.convert('L'))
                # Simple high-pass filter to show potential compression artifacts
                kernel = np.array([[-1,-1,-1],[-1,8,-1],[-1,-1,-1]])
                filtered = cv2.filter2D(gray_img, -1, kernel)
                ax8.imshow(np.abs(filtered), cmap='hot')
                ax8.set_title("8. JPEG Artifacts (Alternative)", fontsize=10, fontweight='bold')
            except:
                ax8.imshow(np.zeros((100,100)), cmap='hot')
                ax8.set_title("8. JPEG Ghost (No Data)", fontsize=10, fontweight='bold')
    else:
        ax8.text(0.5, 0.5, 'JPEG Ghost\nNot Available', ha='center', va='center', transform=ax8.transAxes)
        ax8.set_title("8. JPEG Ghost Detection", fontsize=10, fontweight='bold')
    ax8.axis('off')
    
    # === ROW 3: Combined Analysis & Frequency ===
    # 9. Combined Heatmap
    ax9 = fig.add_subplot(gs[2, 0])
    combined_heatmap = create_advanced_combined_heatmap(analysis_results, original_pil.size)
    ax9.imshow(original_pil, alpha=0.5)
    ax9.imshow(combined_heatmap, cmap='inferno', alpha=0.5)
    ax9.set_title("9. Peta Anomali Gabungan", fontsize=10, fontweight='bold')
    ax9.axis('off')
    
    # 10. Frequency Domain Analysis
    ax10 = fig.add_subplot(gs[2, 1])
    create_frequency_visualization(ax10, analysis_results)
    
    # 11. Texture Analysis
    ax11 = fig.add_subplot(gs[2, 2])
    create_texture_visualization(ax11, analysis_results)
    
    # 12. Statistical Analysis
    ax12 = fig.add_subplot(gs[2, 3])
    create_statistical_visualization(ax12, analysis_results)
    ax12.set_title("12. Channel Entropy Analysis", fontsize=10, fontweight='bold')
    
    # === ROW 4: Quality & Noise Analysis ===
    # 13. JPEG Quality Response
    ax13 = fig.add_subplot(gs[3, 0])
    create_quality_response_plot(ax13, analysis_results)
    ax13.set_title("13. JPEG Quality Response", fontsize=10, fontweight='bold')
    
    # 14. Noise Map
    ax14 = fig.add_subplot(gs[3, 1])
    noise_map = analysis_results.get('noise_map')
    if noise_map is not None:
        if not isinstance(noise_map, Image.Image):
            noise_map = Image.fromarray((noise_map * 255).astype(np.uint8), mode='L')
        ax14.imshow(noise_map, cmap='gray')
    ax14.set_title("14. Noise Distribution Map", fontsize=10, fontweight='bold')
    ax14.axis('off')
    
    # 15. Metadata Table
    ax15 = fig.add_subplot(gs[3, 2])
    create_metadata_table(ax15, analysis_results.get('metadata', {}))
    
    # 16. DCT Coefficients Visualization
    ax16 = fig.add_subplot(gs[3, 3])
    create_dct_visualization(ax16, original_pil)
    ax16.set_title("16. DCT Coefficients", fontsize=10, fontweight='bold')
    
    # === ROW 5: Advanced Fusion Analysis ===
    # 17. MM Fusion Analysis
    ax17 = fig.add_subplot(gs[4, 0])
    create_mm_fusion_visualization(ax17, original_pil, analysis_results)
    
    # 18. TruFor Analysis
    ax18 = fig.add_subplot(gs[4, 1])
    create_trufor_visualization(ax18, original_pil, analysis_results)
    
    # 19. Summary Report
    ax19 = fig.add_subplot(gs[4, 2])
    create_summary_report(ax19, analysis_results)
    ...
    validation_text = f"**20. Pipeline Status**\n\n" \
                      f"Stages: {completed_stages}/{total_stages}\n" \
                      f"Success: {success_rate:.1f}%\n" \
                      f"Failed: {failed_stages_count}"
    ax20.text(0.5, 0.5, validation_text, transform=ax20.transAxes,
              ha='center', va='center', fontsize=10, fontweight='bold',
              bbox=dict(boxstyle='round,pad=0.5', fc='lightgreen' if success_rate > 80 else 'lightyellow', alpha=0.7))
    
    # === ROW 6: Probability & Uncertainty Analysis ===
    # 21. Probability Bars
    ax21 = fig.add_subplot(gs[5, 0:2])  # Span 2 columns
    if 'classification' in analysis_results and 'uncertainty_analysis' in analysis_results['classification']:
        create_probability_bars(ax21, analysis_results)
    else:
        ax21.text(0.5, 0.5, 'Probability Analysis\nNot Available', ha='center', va='center', fontsize=12)
        ax21.axis('off')
    
    # 22. Uncertainty Visualization
    ax22 = fig.add_subplot(gs[5, 2:4])  # Span 2 columns
    if 'classification' in analysis_results and 'uncertainty_analysis' in analysis_results['classification']:
        create_uncertainty_visualization(ax22, analysis_results)
    else:
        ax22.text(0.5, 0.5, 'Uncertainty Analysis\nNot Available', ha='center', va='center', fontsize=12)
        ax22.axis('off')

    try:
        plt.savefig(output_filename, dpi=100, bbox_inches='tight', pad_inches=0.2)
...
def create_mm_fusion_visualization(ax, original_pil, analysis_results):
    """Create MM Fusion analysis visualization"""
    mm_fusion_data = analysis_results.get('mm_fusion_analysis', {})
    
    if mm_fusion_data:
        try:
            # Get fusion data
            heatmap = mm_fusion_data.get('heatmap_data', {})
            confidence = mm_fusion_data.get('forgery_confidence_score', 0)
            detected = mm_fusion_data.get('forgery_detected', False)
            confidence_factors = mm_fusion_data.get('confidence_factors', [])
            
            # Try to create heatmap visualization
            heatmap_created = False
            
            if isinstance(heatmap, dict):
                # Check for heatmap_array (actual data from generate_forgery_heatmap)
                if 'heatmap_array' in heatmap and heatmap['heatmap_array']:
                    try:
                        # Convert list to numpy array
                        heatmap_array = np.array(heatmap['heatmap_array'])
                        if heatmap_array.size > 0 and np.any(heatmap_array > 0):
                            # Overlay on original image for better visualization
                            ax.imshow(original_pil, alpha=0.6)
                            im = ax.imshow(heatmap_array, cmap='hot', alpha=0.4, vmin=0, vmax=1)
                            heatmap_created = True
                    except:
                        pass
            
            # If no heatmap, create confidence-based visualization
            if not heatmap_created:
                # Create gradient based on confidence score
                h, w = 200, 200
                x = np.linspace(0, 1, w)
                y = np.linspace(0, 1, h)
                X, Y = np.meshgrid(x, y)
                
                # Create radial gradient centered on detected areas
                center_x, center_y = 0.5, 0.5
                R = np.sqrt((X - center_x)**2 + (Y - center_y)**2)
                
                # Scale by confidence
                conf_map = (1 - R) * (confidence / 100)
                conf_map = np.clip(conf_map, 0, 1)
                
                # Add some noise for realism
                noise = np.random.normal(0, 0.05, conf_map.shape)
                conf_map = np.clip(conf_map + noise, 0, 1)
                
                # Display
                cmap = 'hot' if detected else 'RdYlGn_r'
                ax.imshow(conf_map, cmap=cmap, vmin=0, vmax=1)
            
            status = "DETECTED" if detected else "CLEAN"
            color = 'red' if detected else 'green'
            ax.set_title(f"17. MM Fusion ({status}: {confidence:.1f}%)", 
                        fontsize=10, fontweight='bold', color=color)
            
        except Exception as e:
            ax.text(0.5, 0.5, f'MM Fusion\nError: {str(e)[:30]}', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title("17. MM Fusion Analysis", fontsize=10, fontweight='bold')
    else:
        # Show confidence factors if available
        confidence_factors = mm_fusion_data.get('confidence_factors', [])
        if confidence_factors:
            text = "MM Fusion Factors:\n\n"
            for i, factor in enumerate(confidence_factors[:5]):
                text += f"{i+1}. {factor[:40]}...\n" if len(factor) > 40 else f"{i+1}. {factor}\n"
            ax.text(0.5, 0.5, text, ha='center', va='center', transform=ax.transAxes, fontsize=8)
        else:
            ax.text(0.5, 0.5, 'MM Fusion\nNo Data Available', 
                   ha='center', va='center', transform=ax.transAxes)
        ax.set_title("17. MM Fusion Analysis", fontsize=10, fontweight='bold')
    
    ax.axis('off')

def create_trufor_visualization(ax, original_pil, analysis_results):
    """Create TruFor forensic analysis visualization"""
    trufor_data = analysis_results.get('trufor_analysis', {})
    
    if trufor_data:
        try:
            # Get TruFor data
            forensic_conf = trufor_data.get('forensic_confidence', 0)
            is_authentic = trufor_data.get('is_authentic', False)
            risk_level = trufor_data.get('risk_level', 'UNKNOWN')
            
            # Get forgery localization map if available
            forgery_map = trufor_data.get('forgery_localization_map')
            
            if forgery_map is not None and isinstance(forgery_map, np.ndarray) and forgery_map.size > 0:
                # Display forgery localization map
                if np.any(forgery_map > 0):
                    ax.imshow(forgery_map, cmap='hot', vmin=0, vmax=np.max(forgery_map))
                else:
                    # If all zeros, show grayscale image with overlay
                    ax.imshow(original_pil, alpha=0.7)
                    ax.imshow(np.zeros(original_pil.size[::-1]), cmap='hot', alpha=0.3)
            else:
                ...
                status_text = "AUTHENTIC" if is_authentic else "SUSPICIOUS"
                ax.text(0.5, 0.5, f"{status_text}\n\nConfidence: {forensic_conf:.1f}%\nRisk: {risk_level}", 
                       ha='center', va='center', transform=ax.transAxes, 
                       fontsize=11, fontweight='bold')
            
            # Set title with color coding
            title_color = 'green' if is_authentic else 'red'
            ax.set_title(f"18. TruFor ({risk_level}: {forensic_conf:.1f}%)", 
                        fontsize=10, fontweight='bold', color=title_color)
            
        except Exception as e:
            ax.text(0.5, 0.5, f'TruFor\nError: {str(e)[:30]}', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title("18. TruFor Analysis", fontsize=10, fontweight='bold')
    else:
        ax.text(0.5, 0.5, 'TruFor Analysis\nNot Available', 
               ha='center', va='center', transform=ax.transAxes)
        ax.set_title("18. TruFor Forensic", fontsize=10, fontweight='bold')
