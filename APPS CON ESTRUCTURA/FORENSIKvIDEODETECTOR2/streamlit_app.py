...
st.set_page_config(
    page_title="ForeInsight Video | Dashboard Forensik Video",
    layout="wide",
...
# Inisialisasi HistoryManager dengan direktori persisten untuk Hugging Face
...
with col1:
    # Menampilkan logo UHO - Path relatif untuk Hugging Face
    logo_path = "Logo-UHO-Normal-1-294x300[1].png"
    try:
        st.image(logo_path, width=120)
    except:
        st.write("🏛️")  # Fallback jika logo tidak ditemukan

with col2:
    st.title("🔎 ForeInsight Video: Sistem Deteksi Forensik Keaslian Video")
    st.markdown("Menggunakan **Metode K-Means** dan **Localization Tampering** dengan Dukungan ELA dan SIFT")
    st.markdown("*Universitas Halu Oleo - Fakultas Teknik*")
...
with st.sidebar:
    ...
    # System Status untuk Hugging Face Spaces
    st.subheader("📊 Status Sistem")
    storage_info = history_manager.get_storage_usage()
    if isinstance(storage_info, dict):
        st.metric("Penyimpanan Digunakan", f"{storage_info.get('total_size_mb', 0)} MB")
        st.metric("Total Analisis", storage_info.get('history_count', 0))
        st.metric("Total Artefak", storage_info.get('artifacts_count', 0))
    
    if st.button("🧹 Bersihkan Artefak Lama", help="Hapus artefak lebih dari 30 hari"):
        with st.spinner("Membersihkan artefak lama..."):
            history_manager.cleanup_old_artifacts()
            st.success("Artefak lama berhasil dibersihkan!")
            st.rerun()

    # Fitur baru: Bersihkan penyimpanan sistem tanpa menghapus riwayat
    if st.button("🧹 Bersihkan Penyimpanan Sistem", help="Hapus folder kerja sementara dan artefak yatim tanpa menghapus riwayat"):
        with st.spinner("Membersihkan penyimpanan sistem..."):
            result = history_manager.cleanup_system_storage()
            freed = result.get('freed_mb', 0.0)
            temp_dirs = result.get('removed_temp_dirs', 0)
            orphans = result.get('removed_orphan_artifacts', 0)
            st.success(f"Selesai. Dibebaskan {freed} MB | Temp: {temp_dirs} | Orphan: {orphans}")
            time.sleep(1)
            st.rerun()
    ...
    if selected_tab == "Analisis Baru":
        ...
        # Batasan ukuran file untuk Hugging Face Spaces
        MAX_FILE_SIZE = 200 * 1024 * 1024  # 200MB
        uploaded_video = st.file_uploader(
            "Unggah Video Bukti (Max 200MB)", 
            type=["mp4", "avi", "mov", "mkv"],
            help="Ukuran file maksimal 200MB untuk performa optimal"
        )
        
        if uploaded_video and uploaded_video.size > MAX_FILE_SIZE:
            st.error(f"Ukuran file terlalu besar! Maksimal {MAX_FILE_SIZE//(1024*1024)}MB")
            uploaded_video = None
            
        baseline_video = st.file_uploader(
            "Unggah Video Baseline (Opsional)", 
            type=["mp4", "avi", "mov", "mkv"],
            help="Video asli untuk perbandingan deteksi penyisipan"
        )
        
        fps = st.number_input("Frame Extraction FPS", min_value=1, max_value=30, value=10, step=1)
        run = st.button("🚀 Jalankan Analisis Forensik", width='stretch', type="primary")
        ...
        # Inisialisasi default untuk opsi
        show_technical_details = True
        show_simple_explanations = True
        ...
        if st.button("🗑️ Hapus Semua Riwayat", width='stretch'):
...
def load_image_as_bytes(path_str: Optional[Union[str, Path]]) -> Optional[bytes]:
    """Load image as bytes with error handling for cloud environment"""
...
def display_history_card(entry):
    """Display history card with error handling"""
    try:
        ...
        with st.container(border=True):
            ...
            total_anomaly = entry.get("summary", {}).get("total_anomaly", 0)
            ...
            elif total_anomaly > 0:
                tag_html = f'<div class="history-anomaly-tags"><span class="history-anomaly-tag" style="background-color: #ffc107; color: #000;">⚠️ {total_anomaly} anomali terdeteksi</span></div>'
                st.markdown(tag_html, unsafe_allow_html=True)
            else:
            ...
            with action_cols[0]:
                if st.button("🔍 Lihat Detail", key=f"view_{entry['id']}", width='stretch', type="primary"):
                    st.session_state.selected_history_id = entry["id"]
                    st.rerun()
            with action_cols[1]:
                if st.button("🗑️ Hapus", key=f"delete_{entry['id']}", width='stretch'):
                    history_manager.delete_analysis(entry["id"])
                    st.toast(f"Analisis untuk '{entry['video_name']}' telah dihapus.")
                    time.sleep(0.5)
                    st.rerun()
    except Exception as e:
        st.error(f"Error menampilkan kartu riwayat: {e}")

def display_history_detail(entry_id):
    """Display history detail with error handling"""
    try:
        ...
        with col2:
            if st.button("⬅️ Kembali", width='stretch'):
                st.session_state.selected_history_id = None
                st.rerun()
        ...
        with tabs[0]:
            ...
            with st.container(border=True):
                ...
                if sum(anomaly_types.values()) > 0:
                    ...
                    st.warning(f"⚠️ Terdeteksi {total_anomaly} anomali, namun distribusi tipe tidak tersedia pada data lama. Detail anomali dapat dilihat di tab 'Investigasi Anomali'.")
            ...
            if not localizations:
                ...
                for i, loc in enumerate(sorted_locs):
                    ...
                    with st.expander(f"{desc['icon']} **Peristiwa #{i+1}: {desc['title']}** (Mulai: {loc['start_ts']:.2f}s, Durasi: {loc.get('duration', 0):.2f}s)", expanded=i==0):
                        color_value = desc["color"]
                        st.markdown(f'<div class="localization-event-card" style="border-left-color: {color_value};">', unsafe_allow_html=True)
                        ...
                        if img_path:
                            ...
                            if img_b64:
                                st.image(img_b64, caption=f"Frame bukti visual untuk peristiwa #{i+1}", width='stretch')
        ...
        with tabs[4]:
            with st.container(border=True):
                ...
                if zip_data:
                    filename = f"ForeInsight_Video_Analysis_{entry_id[:8]}_{entry.get('video_name', 'export').replace(' ', '_')}.zip"
                    st.download_button(
        ...
        st.error(f"Error menampilkan detail riwayat: {e}")
        st.error(traceback.format_exc())

def render_history_page():
    """Render history page with error handling"""
    try:
        ...
        st.error(f"Error menampilkan halaman riwayat: {e}")
        st.error(traceback.format_exc())

def display_ferm_tab_content(entry_or_result):
    """Display FERM tab content with error handling"""
    try:
        ...
        with ferm_tabs[2]:
            ...
            if temp_dist:
                ...
                c2.metric("Ukuran Rata-rata Klaster", f"{temp_dist.get('avg_cluster_size', 0):.1f}")
            ...
            if tech_sev:
                ...
                if tech_sev.get("by_type"):
                    ...
                    if severity_data: st.dataframe(severity_data, width='stretch')
        ...
        st.error(f"Error menampilkan tab FERM: {e}")
        st.error(traceback.format_exc())
...
def display_analysis_result(result, baseline_result=None):
    ...
    try:
        ...
        with tabs[0]:
            ...
            with st.expander("Tampilkan Metadata Video Lengkap"):
                for category, items in result.metadata.items():
                    st.write(f"**{category}**")
                    if isinstance(items, dict):
                        items_str = {k: str(v) for k, v in items.items()}
                        df = pd.DataFrame.from_dict(items_str, orient="index", columns=["Value"])
                        st.table(df)
                    elif isinstance(items, (int, float, str)):
                        st.write(f"Value: {items}")
                    else:
                        st.write(f"Value: {str(items)}")
                ...
                for i, loc in enumerate(result.localizations):
                    ...
                    with st.expander(
                        ...
                        if loc.get("sift_path_bytes"):
                            st.image(loc.get("sift_path_bytes"), caption="Bukti Pencocokan Fitur SIFT+RANSAC", width='stretch')
        ...
        with tabs[5]:
            st.header("Hasil Tahap 5: Laporan")
            st.info("Berikut adalah ringkasan lengkap dari hasil analisis forensik video Anda.")
            
            col1, col2 = st.columns(2)
            
            # Download PDF
            with col1:
                if hasattr(result, 'pdf_report_bytes') and result.pdf_report_bytes:
                    st.download_button(
                        label="📄 Unduh Laporan PDF",
                        data=result.pdf_report_bytes,
                        file_name=f"Laporan_Forensik_{Path(result.video_path).stem}.pdf",
                        mime="application/pdf",
                        type="primary"
                    )
                else:
                    st.warning("Laporan PDF tidak tersedia.")

            # Download DOCX
            with col2:
                if hasattr(result, 'docx_report_bytes') and result.docx_report_bytes:
                    st.download_button(
                        label="📝 Unduh Laporan DOCX",
                        data=result.docx_report_bytes,
                        file_name=f"Laporan_Forensik_{Path(result.video_path).stem}.docx",
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        type="primary"
                    )
                else:
                    st.warning("Laporan DOCX tidak tersedia.")

            # Download ZIP
            if hasattr(result, 'zip_report_bytes') and result.zip_report_bytes:
                st.download_button(
                    label="🗂️ Unduh Arsip ZIP Lengkap",
                    data=result.zip_report_bytes,
                    file_name=f"Arsip_Lengkap_{Path(result.video_path).stem}.zip",
                    mime="application/zip",
                    type="primary"
                )
            else:
                st.warning("Arsip ZIP tidak tersedia.")
            
            # Summary statistics
            st.subheader("📊 Ringkasan Statistik")
            col1, col2, col3 = st.columns(3)
            col1.metric("Total Frame", result.summary.get("total_frames", 0))
            col2.metric("Total Anomali", result.summary.get("total_anomaly", 0))
            col3.metric("Persentase Anomali", f"{result.summary.get('pct_anomaly', 0):.1f}%")
            
            # FERM conclusion
            if hasattr(result, 'forensic_evidence_matrix') and result.forensic_evidence_matrix:
                ferm = result.forensic_evidence_matrix
                if ferm.get("conclusion"):
                    st.subheader("🧠 Kesimpulan FERM")
                    conclusion = ferm.get("conclusion", {})
                    reliability = conclusion.get("reliability_assessment", "Tidak Tersedia")
                    reliability_class, _ = get_reliability_class(reliability)
                    st.markdown(f'<div class="{reliability_class}">{reliability}</div>', unsafe_allow_html=True)
                    
                    if conclusion.get("primary_findings"):
                        st.write("**Temuan Utama:**")
                        for finding in conclusion.get("primary_findings", []):
                            st.markdown(f"- **{finding.get('finding', 'N/A')}** ({finding.get('confidence', 'N/A')})")
                    
                    if conclusion.get("recommended_actions"):
                        st.write("**Rekomendasi Tindakan:**")
                        for action in conclusion.get("recommended_actions", []):
                            st.markdown(f"- {action}")
    except Exception as e:
        st.error(f"Error menampilkan hasil analisis: {e}")
        st.error(traceback.format_exc())

# Main execution
if __name__ == "__main__":
    try:
        # MODE 1: PRIORITAS TERTINGGI - Jika hasil analisis sudah ada di session state, tampilkan.
        # Ini menjaga agar hasil tetap ada di layar saat terjadi eksekusi ulang (misalnya, setelah mengklik tombol unduh).
        if selected_tab == "Analisis Baru" and st.session_state.get('analysis_result') is not None:
            display_analysis_result(st.session_state.analysis_result)

        # MODE 2: Jika tombol 'run' diklik dan video sudah diunggah, mulai analisis baru.
        elif selected_tab == "Analisis Baru" and run and uploaded_video is not None:
            with st.spinner("Sedang melakukan analisis forensik video... Mohon tunggu..."):
                try:
                    # Simpan file yang diunggah ke file temporary
                    ...
                    if baseline_video is not None:
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as tmp_baseline:
                            tmp_baseline.write(baseline_video.getbuffer())
                            baseline_path = tmp_baseline.name
                    
                    # Jalankan fungsi analisis inti
                    result = fv.analyze_video(
                        video_path=video_path,
                        baseline_path=baseline_path,
                        fps=fps,
                        ssim_threshold=ssim_slider if not auto_threshold else None,
                        z_threshold=z_slider if not auto_threshold else None,
                        show_technical_details=show_technical_details,
                        show_simple_explanations=show_simple_explanations,
                        show_advanced_metrics=show_advanced_metrics,
                        show_ferm_details=st.session_state.show_ferm_details
                    )
                    
                    # Simpan hasil ke riwayat
                    video_name = uploaded_video.name
                    additional_info = {
                        "fps_awal": fps, "fps_baru": fps,
                        "ssim_threshold": ssim_slider if not auto_threshold else "auto",
                        "z_threshold": z_slider if not auto_threshold else "auto"
                    }
                    history_manager.save_analysis(result, video_name, additional_info)
                    
                    # **KUNCI**: Simpan hasil ke session_state
                    st.session_state.analysis_result = result
                    
                    # Hapus file temporary setelah analisis
                    os.unlink(video_path)
                    if baseline_path:
                        os.unlink(baseline_path)

                    # Paksa eksekusi ulang untuk masuk ke MODE 1 dan menampilkan hasil
                    st.rerun()
        ...
        # MODE 3: Halaman Riwayat Analisis
        elif selected_tab == "Riwayat Analisis":
            # Kosongkan hasil analisis saat beralih tab untuk menghindari menampilkan hasil lama
            st.session_state.analysis_result = None
            render_history_page()

        # MODE 4: Halaman utama default untuk tab "Analisis Baru"
        else:
             # Kosongkan hasil analisis untuk memastikan awal yang bersih
            st.session_state.analysis_result = None
            st.title("Selamat Datang di ForeInsight Video")
            st.markdown("""
            ### 🔍 Sistem Deteksi Forensik Keaslian Video
            Platform ini dirancang untuk melakukan analisis mendalam terhadap file video guna mendeteksi potensi manipulasi.
            - **Analisis Multi-Lapis**: Menggabungkan beberapa metode untuk hasil yang akurat.
            - **Laporan Rinci**: Menghasilkan laporan PDF dan DOCX yang komprehensif.
            - **Visualisasi Data**: Grafik dan plot untuk interpretasi yang lebih mudah.
            
            #### 🚀 Alur Kerja:
            1. Buka menu **Analisis Baru** di sidebar.
            2. **Unggah video** yang ingin Anda periksa.
            3. (Opsional) Unggah video **baseline (asli)** untuk perbandingan.
            4. Atur **parameter FPS** dan threshold, atau gunakan mode otomatis.
            5. Klik **Jalankan Analisis Forensik** dan tunggu hasilnya.
            6. Unduh laporan dari tab **Laporan**.
            """)
            st.info("Pilih 'Analisis Baru' dari sidebar untuk memulai.", icon="ℹ️")
            
    except Exception as e:
        st.error(f"Terjadi error tak terduga pada aplikasi: {e}")
        st.error(traceback.format_exc())
